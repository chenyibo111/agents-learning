...
- 概念层：理解角色、环境、共享世界状态、私有记忆、事件、tick 和涌现行为。
- 最小实践层：用固定角色和规则推进一个可重复的 tick，并输出事件日志。
- 工程实现层：实现事件驱动模拟、随机 seed、私有/公开状态隔离、持久化回放、规则与 LLM Policy 插拔、规模控制和模拟结果评测。

当前 Demo 只验证状态转移，工程层必须保证模拟可重放、可解释和不会泄露上帝视角。
