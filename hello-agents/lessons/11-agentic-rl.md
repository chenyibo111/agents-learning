...
- 概念层：理解 SFT、轨迹、奖励、优势、偏好优化、GRPO 和 reward hacking。
- 最小实践层：在小型确定性环境中生成轨迹、计算奖励并比较策略，不假装完成大模型训练。
- 工程实现层：构建可复现实验集、轨迹存储、奖励版本、训练/评测分离和安全门禁；在资源允许时接入真实训练框架，并报告数据、显存、成本和效果。

本课工程层的重点是可复现实验管线和奖励审计，不是盲目训练一个大模型。
