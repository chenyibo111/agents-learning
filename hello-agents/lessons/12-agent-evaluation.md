...
- 概念层：理解任务成功率、事实/引用正确性、轨迹、延迟、成本、安全和 LLM-as-Judge 的边界。
- 最小实践层：用固定数据集计算成功率、平均步数和安全违规数。
- 工程实现层：实现版本化评测集、轨迹回放、指标报告、人工校准、模型对比、发布门禁和 CI 回归；硬规则与 Judge 结果必须分开记录。

当前 Demo 只是指标计算入口，工程层要让失败样本可以定位、复现和阻止回归。
