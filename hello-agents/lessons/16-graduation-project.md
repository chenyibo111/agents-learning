...
- 工程问题记录：[ISSUES.md](../projects/16-graduation-project/werewolf_arena/ISSUES.md)。
- 工程修复记录：[FIXES.md](../projects/16-graduation-project/werewolf_arena/FIXES.md)。

## 三层实现标准

- 概念层：完成需求、状态、工具、策略、威胁模型、成功标准和发布边界设计。
- 最小实践层：完成一个离线可运行的端到端流程，覆盖正常、失败和边界样例。
- 工程实现层：完成真实模型/工具适配、持久化和恢复、权限与审批、观测与成本、评测门禁、部署说明和安全检查，形成可交付项目；本项目的 LLM 适配器还应能在超时或瞬时网关失败时有限重试并安全降级。
...
本课真实 LLM 运行还需要关注六项工程约束：阶段化行动协议、别名归一化、本地 JSON Schema 校验、失败后的安全 `noop`、真实模式/Token 成本指标，以及输出 Token 上限和请求重试参数。当前还会为每个 Agent 保存脱敏请求级观测，区分主动 `noop`、格式降级、Schema 失败、弃票和有效行动。每次遇到问题先追加问题记录，修复并验证后再追加修复记录；记录不得包含 API Key、完整 Prompt 或完整模型响应。
